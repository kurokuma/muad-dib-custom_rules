...
const os = require('os');
const { test, assert, assertIncludes, runScan } = require('../test-utils');
...
async function runAIConfigTests() {
  ...
  test('AI-CONFIG: detects curl pipe to shell in .cursorrules', () => {
  ...
  test('AI-CONFIG: detects credential access in CLAUDE.md', () => {
    const tmp = createTempDir();
    try {
      fs.writeFileSync(path.join(tmp, 'package.json'), JSON.stringify({ name: 'test', version: '1.0.0' }));
  ...
  test('AI-CONFIG: detects exfiltration via curl POST', () => {
    const tmp = createTempDir();
    try {
      fs.writeFileSync(path.join(tmp, 'package.json'), JSON.stringify({ name: 'test', version: '1.0.0' }));
  ...
  test('AI-CONFIG: detects env grep for secrets', () => {
    const tmp = createTempDir();
    try {
      fs.writeFileSync(path.join(tmp, 'package.json'), JSON.stringify({ name: 'test', version: '1.0.0' }));
  ...
  test('AI-CONFIG: detects injection in .github/copilot-instructions.md', () => {
    const tmp = createTempDir();
    try {
      fs.writeFileSync(path.join(tmp, 'package.json'), JSON.stringify({ name: 'test', version: '1.0.0' }));
  ...
  test('AI-CONFIG: no false positive on clean project', () => {
    const tmp = createTempDir();
    try {
  ...
  test('AI-CONFIG: compound detection escalates to CRITICAL', () => {
    const tmp = createTempDir();
    try {
      fs.writeFileSync(path.join(tmp, 'package.json'), JSON.stringify({ name: 'test', version: '1.0.0' }));
  ...
  test('AI-CONFIG: detects hidden execution instructions', () => {
    const tmp = createTempDir();
    try {
      fs.writeFileSync(path.join(tmp, 'package.json'), JSON.stringify({ name: 'test', version: '1.0.0' }));
  ...
  test('AI-CONFIG: detects adversarial ai-config-injection sample', () => {
    ...
    const output = runScan(sampleDir, '--json');
    const result = JSON.parse(output);
    const threats = result.threats.filter(t => t.type.startsWith('ai_config_injection'));
  ...
  test('AI-CONFIG: AST detects ai-agent-weaponization sample', () => {
